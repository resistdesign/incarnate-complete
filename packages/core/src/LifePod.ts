...
const getMergedDependencies = (
  depStructure: DependencyDeclaration = {},
  merge?: boolean
) => {
  if (merge === false) {
    return depStructure;
...
export default class LifePod extends HashMatrix
  implements DependencyDeclaration {
  ...
  addDependencyMapChangeHandlers = (
    dependencyMap: ObjectOf<HashMatrix> = {}
  ) => {
    Object.keys(dependencyMap).forEach(k =>
      this.addDependencyChangeHandler(dependencyMap[k])
    );
  };

  removeDependencyMapChangeHandlers = (
    dependencyMap: ObjectOf<HashMatrix> = {}
  ) => {
    Object.keys(dependencyMap).forEach(k =>
      this.removeDependencyChangeHandler(dependencyMap[k])
    );
  };
  ...
  addDependencyMapErrorHandlers = (
    dependencyMap: ObjectOf<HashMatrix | undefined> = {}
  ) => {
    Object.keys(dependencyMap).forEach(k =>
      this.addDependencyErrorHandler(dependencyMap[k])
    );
  };

  removeDependencyMapErrorHandlers = (
    dependencyMap: ObjectOf<HashMatrix | undefined> = {}
  ) => {
    Object.keys(dependencyMap).forEach(k =>
      this.removeDependencyErrorHandler(dependencyMap[k])
    );
  };
  ...
  resolve() {
    ...
    if (!this.resolving) {
      ...
      if (this.factory instanceof Function) {
        const resolvedDependencyDeclaration = this.resolveDependencyMap(
          this.dependencies
        );

        if (typeof resolvedDependencyDeclaration !== 'undefined') {
          try {
            resolvedValue = this.factory(
    ...
    return new Promise((res, rej) => {
      ...
      if (typeof timeoutMS === 'number') {
        timeoutIdentifier = setTimeout(
          () =>
            handlers.onError(
              new Error(LifePod.ERROR_MESSAGES.RESOLUTION_TIMEOUT)
            ),
          timeoutMS
        );
      }
