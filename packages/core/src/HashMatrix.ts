...
export default class HashMatrix extends ConfigurableInstance {
    ...
    _setDefaultName() {
        if (!this.hasOwnProperty('name')) {
            if (typeof (this.constructor as ObjectOf<any>).DEFAULT_NAME === 'string') {
    ...
    getBasePathArray(path: HashMatrixPathPartType = '') {
        return path instanceof Array ?
            [...path] :
            (path === '' ? [] : `${path}`.split(this.pathDelimiter));
    }

    getPathArray(path: HashMatrixPathPartType = '', prefixPath: HashMatrixPathPartType = '') {
    ...
    getPathString(path?: HashMatrixPathPartType, prefixPath?: HashMatrixPathPartType) {
